...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret =  {
        id: commit,
        url: 'https://github.com/BaoyiWan/portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
function renderCommitInfo(data, commits) {
  ...
  // Maximum file length
  // First, group by file and count entries for each
  const fileLengths = d3.rollups(data, (v) => v.length, (d) => d.file);
  const maxFileLength = d3.max(fileLengths, ([file, count]) => count);
...
function renderScatterPlot(data, commits) {
  ...
  const rScale = d3 // adjust these values based on your experimentation
  ...
  //yScale.range([usableArea.bottom, usableArea.top]); change 1//y-axis in reverse direction
  ...
  dots.selectAll('circle').data(sortedCommits, d => d.id).join('circle'); //revise1?????
